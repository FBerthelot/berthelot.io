...
<template>
  ...
  >
    ...
    <img
      src="/assets/mariage/Agnes & Florent-transparent.svg"
      class="logo"
      alt=""
    />
  </div>
...
const { animate, reduceAtTheEnd, mode } = defineProps({
  animate: {
    type: Boolean,
    default: true,
  },
...
const emit = defineEmits(['animationOver'])
...
const everythingIsLoaded = computed(() => {
  return Object.values(images).every((img) => img.loaded)
})
