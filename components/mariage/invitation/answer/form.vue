...
<template>
  <main id="answer-main" class="main">
    <form @submit.prevent="submitForm">
      <h2 class="title">{{ t('hello', { names: invitation.name }) }}</h2>

      <p class="subtitle">
        {{ t('ultimatum', invitation.nbOfPeople) }}
      </p>

      <fieldset id="attending" class="question-container">
        <div class="question">
          {{ t('attending.question', invitation.nbOfPeople) }}
        </div>
        <div class="question-instruction">
          {{ t('attending.instruction', invitation.nbOfPeople) }}
        </div>
        <div class="answers checkboxes">
          ...
          <label>
            ...
            <div class="checkbox-content">
              ...
              <div class="option-title">
                {{ t('attending.cant', invitation.nbOfPeople) }}
              </div>
            </div>
          </label>
        </div>
      ...
      >
        <span class="question">{{
          t('children.question', invitation.children.length)
        }}</span>
        <div class="answers checkboxes checkboxes--small">
          <label>
            <input
      ...
      <p
        ...
        class="subtitle"
      >
        {{ t('meal.subtitle', invitation.nbOfPeople) }}
      </p>
      ...
      <section v-if="!formValues.attending.includes('cant')" class="infos">
        <ul>
          ...
          <li v-if="invitation.invitedTo.party" class="info">
            ...
            <div>
              {{ t('housing.p1', invitation.nbOfPeople) }}
            </div>
          </li>
          <li v-if="invitation.invitedTo.party" class="info">
            <img
              src="../../00_design-system/icons/driver.svg"
              class="info-icon"
              alt=""
            />
            <div>
              <div>{{ t('parker.p1', invitation.nbOfPeople) }}</div>
              <div>{{ t('parker.p2', invitation.nbOfPeople) }}</div>
              <div>{{ t('parker.p3', invitation.nbOfPeople) }}</div>
            </div>
          </li>
        </ul>
      </section>
      ...
      <div id="cgu" class="cgu-container">
        <label class="cgu-label">
          ...
          />
          <div>
            {{ t('cgu.p1', invitation.nbOfPeople) }}
            <ul>
              <li>
                {{ t('cgu.email') }}
              </li>
        ...
        >
          {{
            submitFormStatus.loading
              ? t('submit.loading')
              : t('submit.text', invitation.nbOfPeople)
          }}
        </button>
        <span class="error">
          {{
            submitFormStatus.error
              ? t('submit.error', invitation.nbOfPeople)
              : ''
          }}
        </span>
      </div>
...
const formValues = reactive({
  ...
  cgu: false,
  comment: '',
  ...invitation.people.reduce((acc, peopleName) => {
    return {
      ...acc,
...
const peopleThatCome = computed(() => {
  const res = invitation.people?.filter((name) => {
    return !invitation.children.includes(name)
  })

  if (formValues.plus1 === 'yes' && formValues.plus1Name) {
    res.push(formValues.plus1Name)
  }

  if (formValues.children === 'true' || !invitation.questionOnChildren) {
    res.push(...invitation.children)
  }

  return res
})
const formState = computed(() => {
  const mealErrors = peopleThatCome.value.reduce((acc, name) => {
    return {
      ...acc,
      [name]: invitation.invitedTo.party
        ? !formValues.attending.includes('cant') && !formValues[`meal-${name}`]
          ? 'required'
          : false
        : false,
    }
  ...
  const errors = {
    attending: !formValues.attending?.length ? 'required' : false,
    plus1Invited: invitation.plus1Invited
      ? !formValues.attending.includes('cant') && !formValues.plus1
        ? 'required'
        : false
      : false,
    plus1Name:
      !formValues.attending.includes('cant') &&
      formValues.plus1 === 'yes' &&
      !formValues.plus1Name
        ? 'required'
        : false,
    children:
      !formValues.attending.includes('cant') &&
    ...
    comment: !formValues.comment ? 'required' : false,
    cgu: !formValues.cgu ? 'required' : false,
  }
...
const handleAttendingChange = (e) => {
  if (formValues.attending.includes('cant')) {
    formValues.attending = e.target.value === 'cant' ? [] : [e.target.value]
  } else if (e.target.value === 'cant') {
    formValues.attending = [e.target.value]
  } else if (formValues.attending.includes(e.target.value)) {
    formValues.attending = formValues.attending.filter(
      (val) => val !== e.target.value,
    )
  } else {
    formValues.attending = [...formValues.attending, e.target.value]
  }
}

const submitForm = async () => {
  if (!shouldValidateForm.value) {
    shouldValidateForm.value = true
  }
  if (!formState.value.valid) {
    const firstError = [
      'attending',
      'plus1Invited',
      'plus1Name',
      ...
      return !!formState.value.errors[error]
    })
  ...
  submitFormStatus.value.loading = true

  try {
    await saveInvitationAnswer(
      invitation.id,
      peopleThatCome.value,
      formValues,
      invitation,
    )
  ...
  submitFormStatus.value.loading = false
}
</script>
